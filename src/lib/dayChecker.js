...
const todayInfo = (currDate) => {
  const d = currDate && new Date(currDate) || new Date()
...
const weekendHolidayInfo = (currDateInfo) => {
  const {
    day, month, today } = currDateInfo
...
const dayChecker = (currDate) => {
  const today = todayInfo(currDate)
  const weekendOrHoliday = weekendHolidayInfo(today)
  const { hour, dayName } = today
  const { isWeekendOrHoliday, holidayName } = weekendOrHoliday
